...
- **Reasoning-aware rendering**: When the selected model supports reasoning, the app displays the model's reasoning stream in a smaller, more transparent block labeled “Reasoning”, separate from the final answer. Event types handled include `response.reasoning_summary_text.delta/.done` and `response.reasoning_summary_part.added/.done`.
