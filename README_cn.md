...
- **圖片僅在記憶體處理**: 圖片僅在 RAM 中處理，不會寫入磁碟。純文字 Q/A 對話在每次請求完成後，會寫入 `~/.ghost_ai/logs/<requestId>.log` 以便除錯/檢視。
...
- **簡易對話記憶**: 應用在主程序以純文字維持 Q/A 歷史用於提示組合；每次請求完成後，也會將目前對話文字寫入 `~/.ghost_ai/logs/<requestId>.log`。

### 🔒 **隱私與安全**

- **圖片不持久化**: 截圖僅在 RAM 中處理，不會寫入磁碟
- **對話日誌**: 為方便除錯，應用會在每次分析請求完成後，將目前純文字 Q/A 對話寫入 `~/.ghost_ai/logs/<requestId>.log`
