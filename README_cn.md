...
- **簡易對話記憶**: 應用會在記憶體中以純文字保存 Q/A 歷史；後續提問會附上先前 Q/A 文字以延續語境（不落地）。
...
- **固定熱鍵（全部為全域）**: Ask = Cmd/Ctrl+Enter，語音切換 = Cmd/Ctrl+Shift+Enter，切換隱藏 = Cmd/Ctrl+\\，清除對話 = Cmd/Ctrl+R（同時重置語音狀態），上一則回答 = Cmd/Ctrl+Up，下一則回答 = Cmd/Ctrl+Down（無論透過 HUD 的 Hide 按鈕或熱鍵隱藏，都可以再按熱鍵重新顯示）
...
2. **固定熱鍵（全域）**
   ...
   - **語音（開始/停止錄音）**: `Cmd/Ctrl+Shift+Enter`
...
3. 輸入提示與自訂提示，點擊 Send
4. 在回應泡泡查看結果，必要時點擊 Copy response
...
8. **對話記憶**: 每次回答後會將 `Q:`/`A:` 內容附加到主程序的記憶體字串；下一輪會連同新問題一併送出。
