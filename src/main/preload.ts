...
const api = {
  ...
  onTextInputShow: (handler: () => void) =>
    ipcRenderer.on("text-input:show", () => handler()),
  onTextInputToggle: (handler: () => void) =>
    ipcRenderer.on("text-input:toggle", () => handler()),
  ...
  onAskClear: (handler: () => void) =>
    ipcRenderer.on("ask:clear", () => handler()),

  onAudioToggle: (handler: () => void) =>
    ipcRenderer.on("audio:toggle", () => handler()),
  ...
  onTranscribeStart: (
    ...
    const fn = (_e: any, data: { ok: boolean; sessionId: string }) =>
      handler(data);
  ...
  onTranscribeDelta: (
    ...
    const fn = (_e: any, data: { delta: string; sessionId: string }) =>
      handler(data);
  ...
  onTranscribeDone: (
    ...
    const fn = (_e: any, data: { content: string; sessionId: string }) =>
      handler(data);
  ...
  onTranscribeError: (
    ...
    const fn = (_e: any, data: { error: string; sessionId: string }) =>
      handler(data);
