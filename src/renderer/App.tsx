...
export function App() {
  ...
  const currentPageLabel = historyIndex === null ? 'Live' : `${historyIndex + 1}/${assistantAnswerIndices.length}`;
  ...
  const canRegenerate = useMemo(() => assistantAnswerIndices.length > 0 && !busy && !streaming, [assistantAnswerIndices.length, busy, streaming]);
  ...
  useEffect(() => {
    ...
    const offScroll = api?.onAskScroll?.(({ direction }: { direction: 'up' | 'down' }) => {
      try {
        setVisible(true);
        const containers = Array.from(document.querySelectorAll<HTMLDivElement>('.bn-markdown-viewer'));
    ...
    return () => {
      try { if (typeof offSession === 'function') offSession(); } catch {}
      try { if (typeof offScroll === 'function') offScroll(); } catch {}
      try { if (typeof offPaginate === 'function') offPaginate(); } catch {}
    };
  }, []);
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      unsubscribe = (window as any).ghostAI?.analyzeCurrentScreenStream?.(userMessage, effectiveCustomPrompt, {
        ...
        onDone: ({ content, sessionId: sid }: { requestId: string; content: string; sessionId?: string }) => {
          ...
          lastDeltaRef.current = null;
          activeSessionIdForRequestRef.current = null;
          setHistory((prev) => [...prev, { role: 'user', content: userMessage }, { role: 'assistant', content: content ?? '' }]);
          setHistoryIndex(null);
          if (activeUnsubRef.current) {
            try { activeUnsubRef.current(); } catch {}
            activeUnsubRef.current = null;
      ...
      unsubscribe = (window as any).ghostAI?.analyzeCurrentScreenStream?.(userMessage, effectiveCustomPrompt, {
        ...
        onDone: ({ content, sessionId: sid }: { requestId: string; content: string; sessionId?: string }) => {
          ...
          setHistory((prev) => {
            ...
            if (assistantIdx >= 0 && assistantIdx < copy.length) copy[assistantIdx] = { role: 'assistant', content: content ?? '' } as any;
            return copy;
          });
          if (activeUnsubRef.current) {
            try { activeUnsubRef.current(); } catch {}
  ...
  return (
    <div style={{ ...appRootStyle, display: visible ? 'block' : 'none' }}>
      ...
      <div
        ref={bubbleRef}
        style={{ position: 'absolute', top: bubbleTop, left: bubbleLeft, width: bubbleWidth, pointerEvents: 'auto' }}
      >
        {tab === 'settings' && (
